...
#ifndef Q_PHOTOGRAMMETRY_PLUGIN_HEADER
#define Q_PHOTOGRAMMETRY_PLUGIN_HEADER
...
**/
class qPhotogrammetry : public QObject, public ccStdPluginInterface
{
	Q_OBJECT
	Q_INTERFACES(ccStdPluginInterface)
	//replace qDummy by the plugin name (IID should be unique - let's hope your plugin name is unique ;)
        Q_PLUGIN_METADATA(IID "cccorp.cloudcompare.plugin.qPhotogrammetry")

public:

	//! Default constructor
        explicit qPhotogrammetry(QObject* parent = 0);

	//inherited from ccPluginInterface
        virtual QString getName() const { return "qPhotogrammetry"; }
        virtual QString getDescription() const { return "Reconstruct 3D objects from photos"; }
